...
await выполнить(`(async () => {
  const строки = [
    'Сторони домовилися, що виконавець передає замовнику',
    'результат роботи у строк, погоджений сторонами у',
    'додатку номер один, який є невідʼємною частиною',
    'цього договору та підписується обома сторонами.',
    'Місто Київ',
  ];
  ...
  строки.forEach((с, и) => к.fillText(с, 50, 80 + и * 62 + (и === 4 ? 70 : 0)));
...
проба('всего два абзаца, а не пять строк', абзацы.length === 2, абзацы.length);
