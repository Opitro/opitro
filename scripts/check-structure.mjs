...
for (const file of pages) {
  ...
  if (виднаДорожка) {
    ...
    if (!дорожка) add(url, 'мета', 'нет дорожки BreadcrumbList для поиска');
    else {
      const шаги = дорожка.itemListElement || [];
      if (шаги.length < 2) add(url, 'мета', `в дорожке всего ${шаги.length} шаг(ов)`);
      for (const [i, ш] of шаги.entries()) {
        if (ш.position !== i + 1) add(url, 'мета', `шаги дорожки не по порядку: ${ш.position} вместо ${i + 1}`);
