...
const I = {
  ...
  // Шесть текстовых инструментов сидели на запасном значке -- том, что достаётся всем, кому
  // своего не назначено. Разобраны по РОДУ ДЕЛА, как и задумано: не по одному значку на
  // инструмент, а по одному на занятие. Пять из шести получили четыре семьи, шестой
  // («лишние пробелы») взял готовую squash -- он делает то же, что удаление пустых строк:
  // схлопывает пустоту.
  'буквы': '<path d="M4 17l3.4-9 3.4 9"/><path d="M5.3 14.2h4.6"/><path d="M13.5 12h6.5"/><path d="M17.5 9.2 20.5 12l-3 2.8"/>',
  'сортировка': '<path d="M4 6.5h10M4 12h7M4 17.5h4"/><path d="M18.5 5.5v13"/><path d="m15.8 15.8 2.7 2.7 2.7-2.7"/>',
  'дубли': '<rect x="3" y="4" width="12.5" height="5.5" rx="1.6"/><rect x="3" y="14.5" width="12.5" height="5.5" rx="1.6" opacity=".45"/><path d="m18.2 15.6 3.6 3.6M21.8 15.6l-3.6 3.6"/>',
  'счёт': '<path d="M9.2 4 7.4 20M16.8 4 15 20"/><path d="M4 9h16M4 15h16"/>',
  ...
  'разбор-ссылки': '<path d="M9.5 14.5a3.5 3.5 0 0 0 5 0l3-3a3.5 3.5 0 0 0-5-5l-1 1"/><path d="M14.5 9.5a3.5 3.5 0 0 0-5 0l-3 3a3.5 3.5 0 0 0 5 5l1-1"/><path d="M3 3.5v2M3 18.5v2M21 3.5v2M21 18.5v2" opacity=".5"/>',
  // Пропуск с печатью: токен, который предъявляют и проверяют.
  'пропуск': '<rect x="2.5" y="5" width="19" height="14" rx="2.5"/><circle cx="8.5" cy="11" r="2.2"/><path d="M5 16.2c.6-1.5 2-2.2 3.5-2.2s2.9.7 3.5 2.2"/><path d="M15 10h4M15 13.5h2.5"/>',
  // Отпечаток пальца: то, чем хэш и является.
  'отпечаток': '<path d="M12 3.5c-2.6 0-4.8 1.5-5.9 3.7"/><path d="M4.6 10.5A7.4 7.4 0 0 1 19.4 11c0 1.6-.2 3.1-.6 4.5"/><path d="M8 8.6a4.7 4.7 0 0 1 8.7 2.5c0 2.6-.4 5-1.2 7.2"/><path d="M12 11v1.6c0 2.9-.5 5.7-1.5 8.3"/><path d="M7.4 20.4c1-2.1 1.5-4.4 1.5-6.8"/>',
  ...
  listgen: '<path d="M9 6.5h11M9 12h11M9 17.5h11"/><path d="M4 5.5l1.2 1.2L7.2 4.7"/><path d="M4 11l1.2 1.2L7.2 10"/><path d="M4 16.5l1.2 1.2 2-2"/>',
  squash: '<path d="M4 3.5h16M4 7h10"/><path d="M4 20.5h16M4 17h10"/><path d="M12 8.5v3M10 10.2l2 1.8 2-1.8"/><path d="M12 15.5v-3M10 13.8l2-1.8 2 1.8"/>',
  ...
  прицел: '<path d="M3 8V5.5A2.5 2.5 0 0 1 5.5 3H8M16 3h2.5A2.5 2.5 0 0 1 21 5.5V8M21 16v2.5a2.5 2.5 0 0 1-2.5 2.5H16M8 21H5.5A2.5 2.5 0 0 1 3 18.5V16"/><path d="M7 12h10"/>',
  полосы: '<path d="M3 5v14M6 5v14M9.5 5v10M13 5v14M16 5v10M19 5v14M21.5 5v14"/>',
...
const RULES = [
  [/^list-generator$/, 'listgen'],
  // Точным совпадением: ниже общее правило со словом "remove" для звука, и без
  // якорей оно забрало бы страницу себе.
  [/^remove-empty-lines$/, 'squash'],
  // Первым и точным совпадением: ниже правило со словом "speech" отдаёт всё микрофону,
  // а здесь микрофон ни при чём -- читает страница, а не человек.
  ...
  [/^hash-generator$/, 'отпечаток'],
  [/^jwt-decoder$/, 'пропуск'],
  [/^url-parser$/, 'разбор-ссылки'],
  [/^html-entities$/, 'амперсанд'],
  [/^number-base-converter$/, 'разряды'],
  // Ставим ПЕРЕД общими правилами по рубрике: иначе они уйдут в запасной значок.
  [/^(transliteration|remove-diacritics)$/, 'буквы'],
  [/^sort-lines$/, 'сортировка'],
  [/^remove-duplicate-lines$/, 'дубли'],
  [/^character-counter$/, 'счёт'],
  [/^remove-extra-spaces$/, 'squash'],
  [/^code-diff$/, 'патч'],
  [/^barcode$/, 'полосы'],
...
const ЦВЕТА = {
  ...
  'буквы': '#f0abfc', 'сортировка': '#7dd3fc', 'дубли': '#fbbf24', 'счёт': '#5eead4',
