...
// ---- БЛИЖАЙШЕЕ НАЗВАНИЕ ------------------------------------------------------------------------
//
// Точное значение цвета человеку мало о чём говорит: «#4a7c59» не произносится и не
// запоминается. Название говорит сразу -- «тёмно-зелёный», -- и с него начинается разговор.
//
// Список нарочно короткий: три десятка цветов, которые действительно различают словами.
// Полный набор названий CSS сюда не годится -- в нём десяток оттенков одного бежевого, и
// ответ «палевый жёлто-золотистый» точнее не становится, а доверия не прибавляет.
//
// Ключи латиницей: библиотека языка не знает, называют цвета страницы.
export const ОСНОВНЫЕ = [
  ['black', 0x000000], ['darkgrey', 0x555555], ['grey', 0x808080], ['lightgrey', 0xc0c0c0],
  ['white', 0xffffff], ['red', 0xe02020], ['darkred', 0x8b0000], ['pink', 0xff9fb2],
  ['orange', 0xff8c00], ['brown', 0x8b5a2b], ['beige', 0xe8d6b3], ['gold', 0xd4af37],
  ['yellow', 0xffe000], ['olive', 0x808000], ['lightgreen', 0x8bd66b], ['green', 0x2e9b3f],
  ['darkgreen', 0x14532d], ['teal', 0x008080], ['cyan', 0x35d2e0], ['skyblue', 0x67b7e8],
  ['blue', 0x1f5fd0], ['darkblue', 0x14306e], ['navy', 0x0b1c3a], ['violet', 0x7a4fd0],
  ['purple', 0x6b2d8f], ['magenta', 0xd12f9a], ['coral', 0xff7f50], ['salmon', 0xfa8072],
  ['cream', 0xfdf5e6], ['sand', 0xc2b280],
];

/**
 * Назвать цвет ближайшим из списка. Считаем не «по клеточкам», а с поправкой на глаз:
 * зелёный мы различаем тоньше синего, поэтому у каналов разный вес, а вес красного ещё и
 * зависит от того, тёмный цвет или светлый (приём известен как redmean). Простая разность
 * по трём числам путала бы тёмно-синий с чёрным.
 */
export function ближайшийЦвет({ r, g, b }) {
  let лучший = ОСНОВНЫЕ[0][0], лучшее = Infinity;
  for (const [имя, знач] of ОСНОВНЫЕ) {
    const кr = (знач >> 16) & 255, кg = (знач >> 8) & 255, кb = знач & 255;
    const среднее = (r + кr) / 2;
    const дr = r - кr, дg = g - кg, дb = b - кb;
    const рас = (2 + среднее / 256) * дr * дr + 4 * дg * дg + (2 + (255 - среднее) / 256) * дb * дb;
    if (рас < лучшее) { лучшее = рас; лучший = имя; }
  }
  return лучший;
}
