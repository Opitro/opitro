...
export function drawWaveform(canvas, buffer, label, opts = {}) {
  ...
  if (opts.rich) {
    ...
    for (let c = 0; c < n; c++) {
      ...
      // Поля сверху и снизу: обычная запись не достаёт до края примерно на одну десятую
      // высоты, иначе волна выглядит тесно и налезает на имя файла.
      //
      // Порядок важен: сначала поля, и только потом упор в предел. Если обрезать раньше,
      // громкая запись упирается в плато на 0.78 высоты -- видно срез с зазором до края
      // коробки. Теперь громкое доходит ровно до края, а не обрывается внутри.
      min *= 0.78; max *= 0.78;
      if (min < -1) min = -1;
      if (max > 1) max = 1;
