...
export function createLiveWaveform(canvas, opts = {}) {
  ...
  function draw() {
    ...
    if (режим === 'wave') {
      // НАСТОЯЩАЯ форма звука, а не уровень. Столбики уровня на телефоне почти не шевелились:
      // тихая речь даёт маленький пик, и весь ряд стоял ровной полоской. Здесь рисуется сам
      // колебательный след -- он живой даже на тихом голосе, потому что показывает форму,
      // а не громкость. Владелец: «рисуй волну сам».
      if (!отсчёты || !отсчёты.length) { g.fillRect(0, Math.round(mid), W, Math.max(1, Math.round(dpr))); return; }
      const шаг = отсчёты.length / cols;
      for (let c = 0; c < cols; c++) {
        // Берём наибольшее отклонение в своей доле -- так тонкие всплески не теряются.
        let пик = 0;
