...
export function createLiveWaveform(canvas, opts = {}) {
  ...
  let отсчёты = null;   // сюда кладут настоящую форму звука с микрофона
  let высоты = new Float32Array(0); // высота каждого столбика, идёт к цели плавно
  ...
  function draw() {
    ...
    if (режим === 'wave') {
      // Плавная волна, а не мерцание. Владелец: «мерехтит и всё, не выглядит профессионально».
      // Причина мерцания -- каждый кадр рисовался заново по свежим отсчётам, и столбики
      // прыгали. Здесь у каждого столбика СВОЯ высота, которая ПЛАВНО идёт к новой:
      // быстро вверх (0.5) и медленно вниз (0.12), как у настоящих приборов. Получается
      // текучее движение, которое читается как живой звук, а не как рябь.
  ...
  return {
    /** Положить свежую форму звука (Float32Array с микрофона). */
...
export function createPlayer() {
  ...
  let сКакогоСкрыта = 0;
  // Полминуты. Меньше -- это обычное переключение вкладки, узел такое переживает; дольше --
  // телефон успевает уснуть, и узел уже не воскресить.
  const ДОЛГО = 30000;
  if (typeof document !== 'undefined') {
    // Засекаем, КОГДА страница ушла из виду. Короткое переключение вкладки узлу не вредит,
    // а вот долгий сон телефона убивает его насмерть: человек возвращается, жмёт плей -- и
    // тишина. Владелец: «у конкурента после пробуждения можно нажать на файл и он играет,
    // у нас нет». Поэтому смотрим не на сам факт скрытия, а на его ДЛИТЕЛЬНОСТЬ.
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) { сКакогоСкрыта = Date.now(); return; }
      // Вернулись: пробуем разбудить сразу, вдруг узел ещё жив.
      if (сКакогоСкрыта && Date.now() - сКакогоСкрыта > ДОЛГО) былаСкрыта = true;
      сКакогоСкрыта = 0;
